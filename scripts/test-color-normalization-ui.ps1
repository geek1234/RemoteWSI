...
$uiScript = Join-Path $PSScriptRoot "ui_color_norm_smoke_test.py"
...
$screenshotPath = Join-Path $PSScriptRoot "artifacts\\ui-color-normalization-smoke.png"
...
Write-Host "Starting local viewer for color-normalization UI test..." -ForegroundColor Cyan
...
try {
    ...
    py -3.14 $uiScript --base-url $baseUrl --screenshot-path $screenshotPath
    if ($LASTEXITCODE -ne 0) {
        throw "Color-normalization UI smoke test failed."
    }

    Write-Host "Color-normalization UI smoke test passed." -ForegroundColor Green
